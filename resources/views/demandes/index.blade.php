                ...
                <div class="col-12 col-md-8 text-md-end">
                    <div class="dropdown d-inline-block">
                        ...
                        <div class="dropdown-menu dropdown-menu-end demande-filter-panel p-3"
                            ...
                            <div class="mb-3">
                                <label for="filter-etat"
                                    class="form-label small text-muted">{{ __('demandes.filters.status.eu') }} <small
                                        class="d-block text-muted">{{ __('demandes.filters.status.fr') }}</small></label>
                                <select id="filter-etat" class="form-select" name="etat">
                                    <option value="all" @selected($filters['etat'] === 'all')>
                                        {{ __('demandes.filters.options.all_status') }}</option>
                                    @foreach ($etats as $etat)
                                        <option value="{{ $etat }}" @selected($filters['etat'] === $etat)>
                                            {{ $etat }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="mb-3">
                                ...
                                <select id="filter-urgence" class="form-select" name="urgence">
                                    <option value="all" @selected($filters['urgence'] === 'all')>
                                        {{ __('demandes.filters.options.all_urgencies') }}</option>
                                    @foreach ($urgences as $urgence)
                                        <option value="{{ $urgence }}" @selected($filters['urgence'] === $urgence)>
                                            {{ $urgence }}</option>
                                    @endforeach
                                </select>
                            </div>
                            <div class="row g-2">
                                <div class="col-6">
                                    <label for="filter-date-from"
                                        class="form-label small text-muted">{{ __('demandes.filters.date_min.eu') }}
                                        <small
                                            class="d-block text-muted">{{ __('demandes.filters.date_min.fr') }}</small></label>
                                    <input id="filter-date-from" type="date" class="form-control" name="date_from"
                                        value="{{ $filters['date_from'] }}">
                                </div>
                                <div class="col-6">
                                    <label for="filter-date-to"
                                        class="form-label small text-muted">{{ __('demandes.filters.date_max.eu') }}
                                        <small
                                            class="d-block text-muted">{{ __('demandes.filters.date_max.fr') }}</small></label>
                                    <input id="filter-date-to" type="date" class="form-control" name="date_to"
                                        value="{{ $filters['date_to'] }}">
                                </div>
                            </div>
                            <div class="d-flex gap-2 mt-3">
                                <button type="submit"
                                    class="btn demande-btn-primary flex-grow-1">{{ __('demandes.filters.submit') }}</button>
                                <a href="{{ route('demandes.index') }}"
                                    class="btn demande-btn-outline flex-grow-1">{{ __('demandes.filters.reset') }}</a>
                            </div>
                        </div>
                    </div>
                </div>
                    ...
                    <tr>
                        @php
                            ...
                            $sortHelper = function (string $key) use ($sortState, $directionState, $queryBase) {
                                ...
                                $url = request()->fullUrlWithQuery(
                                    array_merge($queryBase, ['sort' => $key, 'direction' => $nextDir]),
                                );
                        ...
                        <th scope="col">
                            <div class="demande-header-cell">
                                ...
                                <a href="{{ $urlId }}" class="demande-sort-link"
                                    aria-label="{{ __('demandes.table.sort.id') }}">
                                    <i class="bi {{ $iconId }}"></i>
                                </a>
                            </div>
                        </th>
                        ...
                        <th scope="col">
                            <div class="demande-header-cell">
                                ...
                                <a href="{{ $urlDate }}" class="demande-sort-link"
                                    aria-label="{{ __('demandes.table.sort.date') }}">
                        ...
                        <th scope="col">
                            <div class="demande-header-cell">
                                ...
                                <a href="{{ $urlUrg }}" class="demande-sort-link"
                                    aria-label="{{ __('demandes.table.sort.urgency') }}">
                        ...
                        <th scope="col">
                            <div class="demande-header-cell">
                                ...
                                <a href="{{ $urlEtat }}" class="demande-sort-link"
                                    aria-label="{{ __('demandes.table.sort.status') }}">
                        ...
                        <tr>
                            ...
                            @php
                                $urgNormalized = Str::of($demande->urgence ?? '')
                                    ...
                                    ->lower()
                                    ->value();
                                $isHighUrgency = in_array($urgNormalized, [
                                    'elevee',
                                    'élevée',
                                    'high',
                                    'haute',
                            ...
                            <td class="align-middle">
                                @if ($isHighUrgency)
                                    <span class="text-warning me-1"
                                        title="{{ __('demandes.table.urgency_high_hint') }}"
                                        aria-label="{{ __('demandes.table.urgency_high_hint') }}">
                                ...
                                <div class="d-inline-flex gap-2">
                                    ...
                                    @if ($canManageDemande)
                                        <form method="POST" action="{{ route('demandes.destroy', $demande) }}"
                                            ...
                                            @method('DELETE')
                                            <button type="button"
                                                class="btn demande-action-btn text-muted demande-delete-btn"
                                                data-demande-title="{{ $demande->titre }}"
                                                title="{{ __('demandes.actions.delete') }}">
                                                <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16"
                                                    fill="currentColor" class="bi bi-trash" viewBox="0 0 16 16">
                                                    <path
                                                        d="M5.5 5.5A.5.5 0 0 1 6 6v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5m2.5 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5m3 0a.5.5 0 0 1 .5.5v6a.5.5 0 0 1-1 0V6a.5.5 0 0 1 .5-.5" />
                                                    <path
                                                        d="M14.5 3a1 1 0 0 1-1 1H13v9a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V4h-.5a1 1 0 0 1-1-1V2a1 1 0 0 1 1-1H6a1 1 0 0 1 1-1h2a1 1 0 0 1 1 1h3.5a1 1 0 0 1 1 1zM4.118 4 4 4.059V13a1 1 0 0 0 1 1h6a1 1 0 0 0 1-1V4.059L11.882 4zM14.5 2h-13v1h13z" />
                                                </svg>
                                            </button>
                                        </form>
                                    @endif
                                    @if ($demande->etat !== 'Terminé' && $canManageDemande)
                                        <form method="POST" action="{{ route('demandes.validate', $demande) }}"
                                            ...
                                            @csrf
                                            @method('PATCH')
                                            <button type="submit" class="btn demande-action-btn text-success"
                                                title="{{ __('demandes.actions.validate') }}">
...
<script>
    document.addEventListener('DOMContentLoaded', function() {
        ...
        // Export CSV via lien temporaire
