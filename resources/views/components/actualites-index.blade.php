    ...
    <div class="grid md:grid-cols-2 lg:grid-cols-3 gap-6 my-4">
        @foreach($actualites as $actu)
            ...
            <div class="bg-white shadow rounded p-4 my-3">
                ...
                <div>
                    <a class="btn btn-primary fw-bold" href="{{ route('actualites.edit', $actu->idActualite) }}">Modifier</a>

                    <form action="{{ route('actualites.delete', $actu->idActualite) }}" method="POST" style="display:inline;">
                        @csrf
                        @method('DELETE')
                        <button type="submit" class="btn btn-danger fw-bold" onclick="return confirm('Supprimer cette actualité ?')">
                            Supprimer
                        </button>
                    </form>
                </div>
