<x-app-layout>
    ...
    <div class="container">
        ...
        <table id="TableFacture" class="table table-striped nowrap dt-left" style="width:100%">
            <thead>
                <tr>
                    <th class="text-left" data-placeholder="Id">
                        {{ Lang::get('facture.id_facture', [], 'eus') }}
                        @if (Lang::getLocale() == 'fr')
                            <p class="fw-light mb-0">{{ Lang::get('facture.id_facture') }}</p>
                        @endif
                    </th>
                    <th class="text-left" data-placeholder="Titre">
                        {{ Lang::get('facture.titre', [], 'eus') }}
                        @if (Lang::getLocale() == 'fr')
                            <p class="fw-light mb-0">{{ Lang::get('facture.titre') }}</p>
                        @endif
                    </th>
                    <th class="text-left" data-placeholder="Etat">{{ Lang::get('facture.etat', [], 'eus') }}
                        @if (Lang::getLocale() == 'fr')
                            <p class="fw-light mb-0">{{ Lang::get('facture.etat') }}</p>
                        @endif
                    </th>
                    <th class="text-left" data-placeholder="Id Famille">
                        {{ Lang::get('facture.id_famille', [], 'eus') }}
                        @if (Lang::getLocale() == 'fr')
                            <p class="fw-light mb-0">{{ Lang::get('facture.id_famille') }}</p>
                        @endif
                    </th>
                    <th class="text-left" data-placeholder="Date Creation">
                        {{ Lang::get('facture.date_creation', [], 'eus') }}
                        @if (Lang::getLocale() == 'fr')
                            <p class="fw-light mb-0">{{ Lang::get('facture.date_creation') }}</p>
                        @endif
                    </th>
                    <th class="text-left" data-placeholder="Actions">{{ Lang::get('facture.actions', [], 'eus') }}
                        @if (Lang::getLocale() == 'fr')
