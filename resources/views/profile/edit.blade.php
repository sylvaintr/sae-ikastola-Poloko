                                ...
                                <div class="rounded-circle bg-light d-flex align-items-center justify-content-center"
                                    ...
                                    @if ($initial)
                                        <span class="text-dark"
                                            style="font-size: 2rem;">{{ strtoupper(substr($initial, 0, 1)) }}</span>
                                    @else
                ...
                <div class="card border-0 shadow-sm">
                    <div class="card-body p-4">
                        <h2 class="h4 fw-bold mb-4">
                            {{ __('auth.informations_famille') ?? 'Informations de la famille' }}</h2>

                        @if ($user->familles->count() > 0)
                            @foreach ($user->familles as $famille)
                                <div class="mb-4">
                                    <div class="mb-3">
                                        <span class="text-muted small">{{ __('auth.famille_id') ?? 'Famille ID' }}
                                            :</span>
                                    ...
                                    @if ($famille->enfants->count() > 0)
                                        <div class="mb-3">
                                            <span
                                                class="text-muted small d-block mb-2">{{ __('auth.enfants') ?? 'Enfants' }}
                                                :</span>
                                            <ul class="list-unstyled ms-3">
                                                @foreach ($famille->enfants as $enfant)
                                                    <li class="mb-2">
                                                        <span class="fw-semibold">{{ $enfant->prenom }}
                                                            {{ $enfant->nom }}</span>
                                                        @if ($enfant->classe)
                                                            <span class="text-muted small"> -
                                                                {{ $enfant->classe->nom }}</span>
                                                        @endif
                                                        @if ($enfant->dateN)
                                                            <span class="text-muted small d-block">
                                                                {{ __('auth.date_naissance') }}:
                                                                {{ \Carbon\Carbon::parse($enfant->dateN)->format('d/m/Y') }}
                                                            </span>
                                                        @endif
                                                    </li>
                                                @endforeach
                                    ...
                                    @else
                                        <p class="text-muted small">
                                            {{ __('auth.aucun_enfant') ?? 'Aucun enfant enregistré' }}</p>
                                    @endif
                                </div>
                        ...
                        @else
                            <p class="text-muted">
                                {{ __('auth.aucune_famille') ?? 'Aucune famille associée à ce profil' }}</p>
                        @endif
                    </div>
                </div>
            </div>
                                        ...
                                        <tr>
                                            <th scope="col">
                                                <div class="text-center">
                                                    <span
                                                        class="admin-table-heading">{{ __('auth.nom_document') }}</span>
                                                </div>
                                            </th>
                                            <th scope="col">
                                                <div class="text-center">
                                                    <span
                                                        class="admin-table-heading">{{ __('auth.etat_document') }}</span>
                                                </div>
                                            </th>
                                            <th scope="col">
                                                <div class="text-center">
                                    ...
                                    <tbody>
                                        @foreach ($documentsObligatoires as $docOblig)
                                            @php
                                                $etats = [
                                                    'non_remis' => [
                                                        'label' => __('auth.non_remis'),
                                                        'badge' => 'bg-secondary',
                                                    ],
                                                    'remis' => ['label' => __('auth.remis'), 'badge' => 'bg-info'],
                                                    'en_cours_validation' => [
                                                        'label' => __('auth.en_cours_validation'),
                                                        'badge' => 'bg-warning',
                                                    ],
                                                    'valide' => [
                                                        'label' => __('auth.valide_document'),
                                                        'badge' => 'bg-success',
                                                    ],
                                                ];
                                                $etat = $etats[$docOblig->etat] ?? $etats['non_remis'];
                                                $peutUploader = !in_array($docOblig->etat, [
                                                    'en_cours_validation',
                                                    'valide',
                                                ]);
                                            @endphp
                                            <tr>
                                                ...
                                                <td class="text-center">
                                                    <span
                                                        class="badge {{ $etat['badge'] }}">{{ $etat['label'] }}</span>
                                                </td>
                                                <td class="text-center">
                                                    <div class="d-flex gap-2 justify-content-center flex-wrap">
                                                        @if ($docOblig->documentUploaded)
                                                            ...
                                                            @if ($docOblig->documentUploaded->etat !== 'valide')
                                                                <form
                                                                    action="{{ route('profile.document.delete', $docOblig->documentUploaded->idDocument) }}"
                                                            ...
                                                            @endif
                                                        @elseif($peutUploader)
                                                            <button type="button" class="btn admin-btn-upload"
                                                    ...
                                                    <div class="modal fade"
                                                        id="uploadModal{{ $docOblig->idDocumentObligatoire }}"
                                                        tabindex="-1">
                                                        <div class="modal-dialog">
                                                            <div class="modal-content">
                                                                <form action="{{ route('profile.document.upload') }}"
                                                                    method="POST" enctype="multipart/form-data">
                                                                    @csrf
                                                                    <input type="hidden" name="idDocumentObligatoire"
                                                                        value="{{ $docOblig->idDocumentObligatoire }}">
                                                                    ...
                                                                    <div class="modal-body">
                                                                        <div class="mb-3">
                                                                            <label
                                                                                for="document{{ $docOblig->idDocumentObligatoire }}"
                                                                                class="form-label">{{ __('auth.upload_document') }}</label>
                                                                            <input type="file"
                                                                            ...
                                                                            @enderror
                                                                            <small
                                                                                class="text-muted">{{ __('auth.upload_document_hint') }}</small>
                                                                        </div>
                                                                    </div>
                                                                    <div class="modal-footer">
                                                                        <button type="button"
                                                                            class="btn btn-secondary"
                                                                            data-bs-dismiss="modal">{{ __('auth.annuler') }}</button>
                                                                        <button type="submit"
                                                                            class="btn btn-primary">{{ __('auth.upload') }}</button>
                                                                    </div>
                                                                </form>
                                                            </div>
                                                        </div>
